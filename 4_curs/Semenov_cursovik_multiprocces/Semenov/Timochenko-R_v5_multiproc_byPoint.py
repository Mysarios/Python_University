...
def Draw_deformed_plot(W_func,U_func,V_func):
    ...
    fig = plt.figure()
    ax = plt.axes(projection='3d',ylim =(-0.15,0.15),xlim = (-0.15,0.15),zlim = (-0.15,0.15))
    ...
    for index in range(0,Size_visualization*Size_visualization):
        for index_2 in range(0, Size_visualization * Size_visualization):
            ...
            Z_array[index][index_2] =Z_array[index][index_2] - W_func.subs( [(Xx, x_new[index]), (Yy, y_new[index_2])])
...
def N_x_Izo(E, nu12,nu21,ex,ey):
    result = (E / (1 - nu12 * nu21)) * h * (ex + nu12 * ey);
    return result
def N_x_Izo_part(E, nu12,nu21,ex,ey):
    Result = [0]*2
    Result[0] = (E / (1 - nu12 * nu21)) * h * ex;
    Result[1] = (E / (1 - nu12 * nu21)) * h * nu12 * ey;
    return Result
def N_y_Izo(E, nu12,nu21,ex,ey):
    Result = (E / (1 - nu12 * nu21)) * h * (ey + nu21 * ex);
    return Result
def N_y_Izo_part(E, nu12,nu21,ex,ey):
    Result = [0]*2
    Result[0] = (E / (1 - nu12 * nu21)) * h * ey;
    Result[1] = (E / (1 - nu12 * nu21)) * h * nu21 * ex;
    return Result
...
def Bubnov_byPoint(Nx,Ny,Nxy,Mx,My,Mxy,Qx,Qy,Tetta_1,Tetta_2,Q_find):
    ...
    print("Get_Bub")
#Nuton
def Get_Jacobian_Nuton_multi(Function_E, Result_w):
    Jacobian = [0] * N * 5
    Def_Function = [0] * N * 5
    Symbol_Function = [0] * N * 5
    ...
    print("Get_First_Jacobi")
    process = []
    for i in range(0, N * 5):
        Jacobian[i] = [0] * N * 5
        p_buf = multiprocessing.Process(target=get_diff, args=(Function_E, queue,Symbol_Function[i], i,))
        process.append(p_buf)
        #Def_Function[i] = Function_E.diff(Symbol_Function[i])
    for i in range(0,math.floor(((N * 5)) / Count_process)+1):
        for j in range(0,Count_process):
            if i * Count_process + j >= (N * 5):
                break
            #print("process =",i * Count_process + j)
            process[i * Count_process + j].start()
        ...
        process[i * Count_process].join()
        print("p1 - join)")
        for j in range(1, Count_process):
            if i * Count_process + j >=(N * 5):
                break
            #print("process =",i * Count_process + j)
            process[i * Count_process + j].join()
    print('Time %.6f' % (time.time() - timer))
    while queue.qsize() > 0:
        buf = queue.get()
        #print("Get queue =",buf[1]," == ",buf[0])
        Def_Function[buf[1]] = buf[0]
    print("end_First_Jacobi")


    print("get_Second_Jacobi")
    process = []
    for row in range(0, N * 5):
        for column in range(0, N * 5):
            p_buf = multiprocessing.Process(target=get_diff_2arg, args=(Def_Function[row], queue, Symbol_Function[column], row,column))
            process.append(p_buf)
            #Jacobian[row][column] = Def_Function[row].diff(Symbol_Function[column])
    for i in range(0,math.floor((N * 5)*(N) / Count_process)+1):
        for j in range(0,Count_process):
            if i * Count_process + j >=(N)*(N * 5):
                break
            #print("process =",i * Count_process + j)
            process[i * Count_process + j].start()
    ...
    while queue.qsize() > 0:
        buf = queue.get()
        #print("Get queue =",buf[1],",",buf[2]," == ",buf[0])
        Jacobian[buf[1]][buf[2]] = buf[0]

    print("end_Second_Jacobi")
    return Jacobian
...
def Ne_Lin_Function_Loop(w_coefs,Es_Get,W_Function_get,U_function, V_function, W_Function, PsiX_function, PsiY_function,Sigma_x,Sigma_y,Sigma_tay):
    ...
    return W_Result
def Nuton_byPoint(w_coefs,Es_Get,W_Function_get,U_function, V_function, W_Function, PsiX_function, PsiY_function,Sigma_x,Sigma_y,Sigma_tay):
    #symbols
    Ee = Symbol('E')
    Hh = Symbol('h')
    print("Start loop")
    ...
    #print("count =",math.floor(count_int / Count_process))

    for i in range(0,math.floor(count_int / Count_process)+1):
        for j in range(0,Count_process):
            if i * Count_process + j >=count_int:
                break
            #print("process =",i * Count_process + j)
            process[i * Count_process + j].start()
        ...
        process[i * Count_process].join()
        print("p(",i * Count_process," - join)")
        for j in range(0, Count_process):
            if i * Count_process + j >=count_int:
                break
            #print("process =",i * Count_process + j)
            process[i * Count_process + j].join()
            print("p(",i * Count_process + j, " - join)")
    ...
    New_Buf_Function = Buf_Function + Es[7]
    W_Result[1] = Nuton_Iter(New_Buf_Function, eps, W_Result[0], w_coefs)
...
def Nuton_Iter(Function_E, eps, w0, w_coefs):
    ...
    if N ==1:
        ...
        Jacobi = Get_Jacobian_Nuton_multi(Function_E, Res_now)
    print("Jacobi =",Jacobi)
    ...
    while (Now_eps > eps):
        ...
        for row in range(0, N * 5):
            for column in range(0, N * 5):
                for W_coefs in range(0, N * 5):
                    Jacobi_buf[row][column] = Jacobi_buf[row][column].subs((Symbol_Function[W_coefs]), (Res_now[W_coefs]))
        Jackobi_Matrix = sym.Matrix(Jacobi_buf)
        Jacobi_Invariant = Jackobi_Matrix.inv()
        print("End Jacobi")
    ...
    if integral_type ==1:
        ...
        if Change == 2 or Change == 3:
            ...
            if part ==1:
                ...
                Es_main[4] = (1/2) * (NXY_I + NXY_I)*F_XY
                #print("Es_main[2] = ", Es_main[2])
        ...
        if Type_Resolve == 1:
            print("Start Nelin")
            W_values = Nuton_byPoint([0]*N*5, Es_main_buf,W_function,U_function, V_function, W_function, PsiX_function, PsiY_function
