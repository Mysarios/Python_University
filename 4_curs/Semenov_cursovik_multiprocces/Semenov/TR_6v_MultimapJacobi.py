...
def Get_New_iterarion_Nuton(Def_function_get, Jackobi_inv, Last_step_X, a):
    ...
    if new_step == 1:
        ...
        step+=step/10
    return Result,step
def Nuton_Loop(Es_Get,W_Function_get):
    print("Start loop")
    Q_now = 0.0001
    ...
    q_for_graph_4 = []
    w_for_graph_4 = []
    q_for_graph_4.append(0)
    w_for_graph_4.append(0)

    W_Result = [0] * (1000)
    W_Result[0] = [0.000001] * N*5

    Q_function = q_function(Q_now,0)
    #print("Q_func =",Q_function)
    ...
    j=1
    # Цикл по разным q
    while (Q_now < Q_step*Q_Count_Steps):
        print("\nIteration =", j)
        print("Q_now = ", Q_now,"\n")
        New_Buf_Function = Buf_Function.subs('q',Q_now)
        ...
        if w_2 < w_for_graph[len(w_for_graph)-1]:
            Q_now -=Q_step/2
        else:
            w_for_graph.append(w_2)
            q_for_graph.append(Q_now - Q_step)
            w_for_graph_4.append(w_4)
            q_for_graph_4.append(Q_now - Q_step)
    ...
    plt.plot(w_for_graph, q_for_graph)
    plt.plot(w_for_graph_4, q_for_graph_4)
    plt.show()
    return W_Result
    ...
    if integral_type ==1:
        ...
        if One_point == 1:
            ...
            if Type_Resolve == 1:
                ...
                W_values = Nuton_Loop(Es_main_buf,W_function)
            if Type_Resolve == 2:
                print("Start Bubnov")
