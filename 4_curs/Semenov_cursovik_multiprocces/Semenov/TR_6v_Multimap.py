...
def intagrete_Es_Simp_2(data):
    ...
    integrate_x += Result.subs(Xx,0)
    integrate_x += Result.subs(Xx,A_lenght_x)
    integrate_x = integrate_x.evalf(10)
    ...
    for Index in range(1, Intervals_count):
        if Index % 2:
            ...
            integrate_x += 2 * Result.subs(Xx, Index * h1)
    integrate_x *= h1/3
    Buf_function = integrate_x.evalf(10)
    ...
    integrate_y += Buf_function.subs(Yy, 0)
    integrate_y += Buf_function.subs(Yy, B_lenght_y)
    integrate_y = integrate_y.evalf(10)
...
def Get_Jacobian_Nuton_multi(Function_E, Result_w):
    ...
    for i in range(0, N * 5):
        #print("Jacobi_first get =", p_result[i])
        Def_Function[i] = p_result[i]
    print('First_Jacobi = Time %.6f' % (time.time() - timer))
    ...
    for row in range(0, N * 5):
        ...
        for column in range(0, N * 5):
            Jacobian[row].append([])
            #print("append data =[",row,"][",column,"] ")
            Data.append((Def_Function[row], Symbol_Function[column],  row, column))

    #print("data size =",Data[399])
    #print("Procces size =", 25*N*N)
    #p_result = []*N*N+1
    #for process_index in range(1,N*N+1):
        #Data_buf =[]
        #for index_data in range((process_index-1)*N*N,process_index*N*N):
            #Data_buf.append(Data[index_data])
        #with mp.Pool(process_index*N*N) as p:
            #p_result = p.map(get_diff_2arg_2, Data)

        #for row in range(0, N * 5):
            #for column in range(0, N * 5):
                #print("Jacobi_first get =", p_result[i])
                #Jacobian[row][column] = p_result[row * (N * 5) + column]
    with mp.Pool(60) as p:
        p_result = p.map(get_diff_2arg_2, Data)
        #print("Len =" ,len(p_result))

    for row in range(0, N * 5):
        for column in range(0, N * 5):
    # print("Jacobi_first get =", p_result[i])
            #print(p_result[row * (N * 5) + column][1])
            Jacobian[row][column] = p_result[row * (N * 5) + column][0]

    print('Second_Jacobi = Time %.6f' % (time.time() - timer))
...
def Get_New_iterarion_Nuton(Def_function_get, Jackobi_inv, Last_step_X, a):
    ...
    #print("First DF =",Def_function_get)
    Symbol_Function = [0] * N * 5
    #print( "A = ", a)
    for i in range(0, N):
        Symbol_Function[i] = 'w' + str(i + 1)
    ...
    # print("Ja =",Jackobi_inv)
    # print("DF =",Def_Function)
    Result = W - a * (Jackobi_inv * Result_function)
    #W = W * Jackobi_inv
    #print("Result = ", Result)
    #print("End DF =", Def_function_get)
    return Result
...
def Nuton_byPoint(w_coefs,Es_Get,W_Function_get,U_function, V_function, W_Function, PsiX_function, PsiY_function,Sigma_x,Sigma_y,Sigma_tay):
    ...
    timer = time.time()
    Es_Get[7] = (-2) * (Q_now * W_Function_get)
    Es = Es_Get.copy()
    p_result =[]
...
def Nuton_Iter(Function_E, eps, w0, w_coefs):
    ...
    #else:
    timer = time.time()
    Def_Function,Jacobi = Get_Jacobian_Nuton_multi(Function_E, Res_now)
    ...
    Count_iteration = 1
    while (Now_eps > eps):
        #print("\n Iteration =",Count_iteration," and eps =",Now_eps," \n")
        timer = time.time()
        Count_Iterions += 1
        ...
        Symbol_data = []
        for W_coefs in range(0, N * 5):
            Symbol_data.append((Symbol_Function[W_coefs],Res_now[W_coefs]))
        #print("Symb(0) = ",Symbol_data[0])
        #print("Jacobi(0) =",Jacobi_buf[0][0])

        Data = []
        for row in range(0, N * 5):
            Jacobi_new.append([])
            for column in range(0, N * 5):
                Jacobi_new[row].append([])
        ...
        print('Subs jacodi = Time %.6f' % (time.time() - timer))
        timer = time.time()
        Jackobi_Matrix = sym.Matrix(Jacobi_new)
        #print('Jackobi_Matrix = Time %.6f' % (time.time() - timer))
        timer = time.time()
        Jacobi_Invariant = Jackobi_Matrix.inv()
        #print('Jacobi_Invariant = Time %.6f' % (time.time() - timer))
        #print("End Jacobi")

        Max_eps = 0
        #print("Def_Function(0) =", Def_Function[0])
        timer = time.time()
        Res_new = Get_New_iterarion_Nuton(Def_Function, Jacobi_Invariant, Res_now, Check_Loop)
